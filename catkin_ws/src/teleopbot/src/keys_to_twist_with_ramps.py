...
def ramped_vel(v_prev, v_target, t_prev, t_now, ramp_rate):
	...
	if error < step:	#we can get there within this timestep
		return v_target
	else:
...
def send_twist():
	global g_last_twist_send_time, g_target_twist, g_last_twist, g_vel_scales, g_vel_ramps, g_twist_ramps, g_twist_pub
...
def fetch_param(name, default):
	if rospy.has_param(name):
		...
		print "parameter [%s] not defined. Defaulting to %.1f" %(name, default)
		return default
